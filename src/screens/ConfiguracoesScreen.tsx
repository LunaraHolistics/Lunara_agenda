...
export default function ConfiguracoesScreen({ onBack }: ConfiguracoesProps) {
  ...
  const { confirmAction, repairDatabase, exportarBackup, importarBackup, resetSystem, clientes, pacotes, updatePacote } = useAppContext();
  
  const limparDadosFantasmas = () => {
    const now = new Date();
    const currentMonthStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    
    const oldActive = (pacotes || []).filter(p => p.status === 'Ativo' && p.mesReferencia < currentMonthStr);
    
    if (oldActive.length > 0) {
        oldActive.forEach(p => updatePacote({...p, status: 'Concluido'}));
        showStatus('success', `Limpeza concluída! ${oldActive.length} pacotes antigos foram desativados.`);
    } else {
        showStatus('success', 'Nenhum dado fantasma encontrado.');
    }
  };
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[var(--color-surface-light)] dark:bg-[var(--color-surface-dark)] p-4 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm">
              ...
              <div className="flex flex-col gap-2">
                ...
                <button 
                  onClick={limparDadosFantasmas}
                  className="w-full py-2.5 bg-indigo-500 text-white font-medium rounded-xl hover:opacity-90 transition-opacity"
                >
