...
export default function FinanceiroScreen({ onBack }: FinanceiroProps) {
  ...
  const filteredTransacoes = useMemo(() => {
    ...
    return combined.filter(t => {
      ...
      // Filtro de valor zero ou data inválida
      if (Number(t.valor) <= 0 || !t.data || isNaN(new Date(t.data + 'T12:00:00').getTime())) return false;
                      ...
                      onClick={() => {
                        if ((t as any).isDespesaState) {
                          ...
                          confirmAction('Tem certeza que deseja excluir esta transação?', () => {
                            // Se for transação de agendamento ou pacote, resetar status para Pendente
                            if (t.agendamentoId) {
                                const ag = agendamentos.find(a => a.id === t.agendamentoId);
                                if (ag) updateAgendamento({...ag, statusPagamento: 'Pendente'});
                            }
                            if (t.pacoteId) {
                                const p = pacotes.find(pa => pa.id === t.pacoteId);
                                if (p) updatePacote({...p, statusPagamento: 'Pendente'});
                            }
