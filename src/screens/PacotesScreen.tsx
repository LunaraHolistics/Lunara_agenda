...
export default function PacotesScreen() {
  ...
  const handleSave = () => {
    ...
    if (editingPacoteId) {
      ...
      // ✅ CORREÇÃO: Apenas atualizar transação existente (não criar nova)
      ...
      if (transacaoExistente) {
        ...
        updateTransacao({
          ...
          data: dataPagamento || transacaoExistente.data,
          metodo: formaPagamento || 'PIX',
          status: statusPagamento
        });
      }
    } else {
      // ✅ CORREÇÃO: addPacote já cria transação automaticamente no AppContext
      addPacote(pacoteData);
      
      // Atualizar a transação recém-criada com dados corretos
      setTimeout(() => {
        const transacaoNova = (transacoes || []).find(t => t.pacoteId === pacoteId);
        if (transacaoNova) {
          const cliente = (clientes || []).find(c => c.id === clienteId);
          const clienteNome = cliente?.nome || 'Cliente';
